...
namespace MonolithEngine
{
  public class LDTKMap
  {
    ...
    public HashSet<EntityInstance> ParseLevel(AbstractScene scene)
    {
      ...
      HashSet<EntityInstance> level1 = new HashSet<EntityInstance>();
      ...
      {
        //RnD
        int pxWid = (int) level2.PxWid; //1400
        ...
        if (level2.Identifier.Equals(scene.GetName()))
        {
          Logger.Debug("Parsing level: " + level2.Identifier);
          float num = 0.0f;
          Array.Sort<LayerInstance>(level2.LayerInstances, 
              new Comparison<LayerInstance>(LDTKMap.CompareLayerInstances));
          foreach (LayerInstance layerInstance in level2.LayerInstances)
          {
            ...
            {
              TileGroup tileGroup = !identifier.StartsWith(this.BACKGROUND) ? (!identifier.StartsWith(this.FOREGROUND) ? this.tileGroup : this.mergedForegroundTileGroup) : this.mergedBackgroundTileGroup;
              int grid = Config.GRID;
              Rectangle sourceRectangle = new Rectangle((int) gridTile.Src[0], (int) gridTile.Src[1], grid, grid);
            ...
            if (layer != null && !identifier.StartsWith(this.BACKGROUND) && !identifier.StartsWith(this.FOREGROUND))
            {
      ...
      foreach (Level level in this.world.Levels)
      {
        if (!(level.Identifier != levelName))
        {
          ...
          foreach (LayerInstance layerInstance in level.LayerInstances)
          {
            ...
            {
              Texture2D texture2D = Assets.GetTexture2D(this.GetMonoGameContentName(layerInstance.TilesetRelPath));
              foreach (TileInstance gridTile in layerInstance.GridTiles)
              {
                long t = gridTile.T;
