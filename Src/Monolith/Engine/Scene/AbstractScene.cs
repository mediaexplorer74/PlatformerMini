...
namespace MonolithEngine
{
  public abstract class AbstractScene : IScene
  {
    ...
    {
      this.SceneName = sceneName != null && sceneName.Length != 0 
                ? sceneName : throw new Exception("The scene must have a non-null, non-empty unique name!");
    ...
    public void Draw(SpriteBatch spriteBatch)
    {
      ...
      foreach (Camera camera in this.Cameras)
      {
        ...
        spriteBatch.Begin(samplerState: SamplerState.PointClamp, 
            transformMatrix: new Matrix?(camera.GetUITransformMatrix()));
