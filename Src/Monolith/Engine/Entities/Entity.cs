...
namespace MonolithEngine
{
  public class Entity : GameObject, IColliderEntity, IHasTrigger, IGameObject, IRayBlocker
  {
    ...
    protected virtual void SetRayBlockers()
    {
      ...
      this.RayBlockerLines.Add((this.Transform.Position, 
          new Vector2(this.Transform.X, this.Transform.Y + (float) Config.GRID)));
      this.RayBlockerLines.Add((this.Transform.Position, 
          new Vector2(this.Transform.X + (float) Config.GRID, this.Transform.Y)));
      this.RayBlockerLines.Add((new Vector2(this.Transform.X + (float) Config.GRID, this.Transform.Y), 
          new Vector2(this.Transform.X + (float) Config.GRID, this.Transform.Y + (float) Config.GRID)));
      this.RayBlockerLines.Add((new Vector2(this.Transform.X, this.Transform.Y + (float) Config.GRID), 
          new Vector2(this.Transform.X + (float) Config.GRID, this.Transform.Y + (float) Config.GRID)));
    }
