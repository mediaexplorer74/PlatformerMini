...
namespace MonolithEngine
{
  public abstract class MonolithGame : Game
  {
    ...
    public MonolithGame()//(MonolithEngine.Platform targetPlatform)
    {
      ...
      Config.GRAVITY_FORCE = 0.1f;//0.3f;
      Config.JUMP_FORCE = 6f; //1f;
    ...
    protected override void Update(GameTime gameTime)
    {
      ...
      else
      {
        ...
        for ( this.accumulator += num; 
                (double) this.accumulator >= (double) this.fixedUpdateDelta; 
                this.accumulator -= this.fixedUpdateDelta )
        {
          this.FixedUpdate();
